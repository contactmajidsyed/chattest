...
export default class ChatTranscriptor extends PureComponent {
  ...
  renderMessage = (itemDetails, isLatestMessage, prevItem) => {
      ...
      >
        {(this.props.contactStatus === CONTACT_STATUS.CONNECTED ||
          ...
          this.props.contactStatus === CONTACT_STATUS.ENDED) && (
            <TranscriptBody>
              {this.props.transcript.map((item, idx, arr) => {
                let prevItem = (idx !== 0) ? arr[idx-1] : {};
                return this.renderMessage(item, idx === lastMessageIndex, prevItem)})}
